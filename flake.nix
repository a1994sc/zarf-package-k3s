      ...
      let
        ...
        buildInputs = self.checks.${system}.pre-commit-check.enabledPackages ++ [
          (pkgs.writeShellScriptBin "zpkg" ''
            ...
            zarf package create -o $(git rev-parse --show-toplevel) --log-format=console --confirm $@
          '')
          (pkgs.writeShellScriptBin "zdeploy" ''
            rm -rf $TMPDIR/zarf-*
            zarf package deploy --confirm --log-format=console $@
          '')
          pkgs.gh
        ];
      in
